...
/**
 * Custom deserializer for a PROV attribute map encoded as: {"{qualifiedName}": [{attribute}, {attribute}, ... ], ...}
 *
 * Why needed: - JSON object keys are qualified names that must be resolved using the active Namespace. - Attribute parsing depends on the owning element name (the key), which
 * Jackson does not provide to standard deserializers for map values. This class injects namespace context and the owning element name while building a Map<QualifiedName,
 * Set<Attribute>>.
 **/
  ...
  public Map deserialize(JsonParser jp, DeserializationContext deserializationContext) throws IOException {
    ...
    while (it.hasNext()) {
      ...
      while (elements.hasNext()) {
        ...
        Attribute attr = new CustomAttributeDeserializerWithRootName().deserialize(elementName, next,
